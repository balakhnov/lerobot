...
Memory-bound? Drop the microbatch size (~linear) and use
`--gradient_accumulation_steps=N` to recover the effective batch, or for SmolVLA leave
`freeze_vision_encoder=True`. The effective global batch is
...
Here, a training `step` is a successful optimizer update. For example, a single GPU with
`--batch_size=2 --gradient_accumulation_steps=4` has an effective batch size of 8 while retaining
the approximate activation-memory footprint of batch size 2.
