...
Since the effective batch size increases with multiple GPUs or gradient accumulation
(`batch_size × num_gpus × gradient_accumulation_steps`), you may want to reduce the number of
training steps proportionally:
...
To retain the effective batch while reducing per-GPU activation memory, reduce `batch_size` and
increase `gradient_accumulation_steps`:

```bash
# 2 GPUs × batch size 2 × 4 accumulated microbatches = effective batch size 16
accelerate launch --num_processes=2 $(which lerobot-train) \
  --batch_size=2 \
...
`steps`, logging frequencies, evaluation frequencies, checkpoint frequencies, and learning-rate
scheduler steps all count successful optimizer updates, not individual microbatches. Checkpoints
store the accumulation count so sampler resume remains aligned even if it is later overridden.
...
- The effective batch size is `batch_size × num_gpus × gradient_accumulation_steps`. If you use 4 GPUs with `--batch_size=8 --gradient_accumulation_steps=2`, your effective batch size is 64.
- Gradient accumulation with FSDP is not supported yet. LeRobot raises an exception when FSDP is combined with `gradient_accumulation_steps > 1`; use `1` for FSDP training.
